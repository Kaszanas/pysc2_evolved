...
See the [scripted agents](../pysc2_evolved/agents/scripted_agent.py) for an example of
...
`ValidActions` in `pysc2_evolved.lib.actions`, and then each observation specifies which
of the available function is valid this frame. Each action is a single
`FunctionCall` in `pysc2_evolved.lib.actions` with all its arguments filled.

The full set of types and functions are defined in `pysc2_evolved.lib.actions`. The set
...
function call. The exact definitions are in `pysc2_evolved.lib.actions.TYPES`
...
In `pysc2_evolved.lib.actions.FUNCTIONS` specific functions have an additional parameter
that references the general parameter.

#### Example usage

Take a look at the [random agent](../pysc2_evolved/agents/random_agent.py) for an
...
The main SC2 environment is at `pysc2_evolved.env.sc2_env`, with the action and
observation space defined in `pysc2_evolved.lib.features`.

The most important argument is `map_name`, which is how to find the map.
Find the names by using `pysc2_evolved.bin.map_list` or by looking in `pysc2_evolved/maps/*.py`.
