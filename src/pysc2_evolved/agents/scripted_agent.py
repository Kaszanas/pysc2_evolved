...
class CollectMineralShardsFeatureUnits(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        if not marines:
            return FUNCTIONS.no_op()
        marine_unit = next(
            (m for m in marines if m.is_selected == self._marine_selected), marines[0]
        )
...
class CollectMineralShardsRaw(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        marines = [
            unit for unit in obs.observation.raw_units if unit.alliance == _PLAYER_SELF
        ]
        if not marines:
...
class DefeatRoachesRaw(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        roaches = [
            unit for unit in obs.observation.raw_units if unit.alliance == _PLAYER_ENEMY
        ]
