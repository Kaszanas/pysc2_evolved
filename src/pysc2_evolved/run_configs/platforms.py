        ...
        with open(path, "rb") as f:  # Binary because the game appends a '\0' :(.
            for line in f:
                ...
                if len(parts) == 2 and parts[0] == "executable":
                    ...
                    # REVIEW: This is a stupid way to get the parent directory three times...
                    # Just pathlib.Path(exec_path).parents[3] would be better (with a check for out of bounds).
...
class LocalBase(lib.RunConfig):
    ...
    def start(self, want_rgb: bool = True, **kwargs):
        """Launch the game."""
        del want_rgb  # Unused
    ...
    def get_versions(self, containing: str | None = None) -> dict[str, lib.Version]:
        ...
        for directory in list_of_dirs:
            if not directory.startswith("Base"):
                if directory.startswith(version_prefix.lower()):
                    # Rename the directory to match the prefix.
                    new_dir = os.path.join(
                        versions_dir,
                        directory.replace(version_prefix.lower(), version_prefix),
                    )
                    os.rename(os.path.join(versions_dir, directory), new_dir)

        versions_found = sorted(
            int(v[len(version_prefix) :])
        ...
        known_versions = [
            v for v in lib.VERSIONS.values() if v.build_version in versions_found
        ]
        ...
        # versions newer than what are known by pysc2_evolved, and so is the default.
        known_versions.append(lib.Version("latest", max(versions_found), None, None))
        ret = lib.version_dict(known_versions)
        if containing is not None and containing not in ret:
            raise ValueError(
                "Unknown game version: %s. Known versions: %s."
...
class Windows(LocalBase):
    ...
    def __init__(self, version: Version | str | None = None):
        exec_path = (
            os.environ.get("SC2PATH")
            or _read_execute_info(os.path.expanduser("~/Documents"), 3)
...
class MacOS(LocalBase):
    ...
    def __init__(self, version: Version | str | None = None):
        exec_path = (
            os.environ.get("SC2PATH")
            or _read_execute_info(
                os.path.expanduser("~/Library/Application Support/Blizzard"), 6
            )
            or "/Applications/StarCraft II"
        )
        super(MacOS, self).__init__(
            exec_path, "SC2.app/Contents/MacOS/SC2", version=version
        )
...
class Linux(LocalBase):
    ...
    def __init__(self, version: Version | str | None = None):
        ...
        env["LD_LIBRARY_PATH"] = ":".join(
            filter(
                None,
                [os.environ.get("LD_LIBRARY_PATH"), os.path.join(base_dir, "Libs/")],
            )
        )
