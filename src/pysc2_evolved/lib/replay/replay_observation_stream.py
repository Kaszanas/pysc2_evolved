...
class ReplayObservationStream(object):
    ...
    def _get_controllers(self, version):
        ...
        if not self._run_config or self._run_config.version != version:
            ...
            self._controllers = [proc.controller for proc in self._sc2_procs]
    ...
    def start_replay_from_data(self, replay_data, player_id: int):
        ...
        if info.local_map_path and info.local_map_path.lower().endswith(".sc2map"):
