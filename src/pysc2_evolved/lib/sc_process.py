...
class StarcraftProcess(object):
    ...
    def _check_exists(self, exec_path: str):
    ...
    def _launch(self, run_config: RunConfig, args, **kwargs):
        ...
        try:
            ...
            raise SC2LaunchError(f"Failed to launch: {str(args)}") from e

    def _shutdown(self):
        """Terminate the sub-process."""
        if self._proc:
            ret = _shutdown_proc(self._proc, 3)
            logging.info(f"Shutdown with return code: {str(ret)}")
            self._proc = None
