...
def with_lock(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def _with_lock(*args: Any, **kwargs: Any) -> Any:
            with lock:
...
def clamp(n: int | float, smallest: int | float, largest: int | float) -> int | float:
...
class _Ability(
    ...
    def __new__(
        ...
        return super(_Ability, cls).__new__(
            ...
            name=(
                general_data.friendly_name
                or general_data.button_name
                or general_data.link_name
            ),
...
class _Surface(object):
    ...
    def draw_line(
        self,
        color: colors.Color | tuple[int, int, int],
        start_loc: point.Point,
        end_loc: point.Point,
        thickness: int = 1,
    ) -> None:
    ...
    def draw_arc(
        self,
        color: colors.Color | tuple[int, int, int],
        world_loc: point.Point,
        world_radius: point.Point,
        start_angle: float,
        stop_angle: float,
        thickness: int = 1,
    ):
    ...
    def blit_np_array(self, array: np.ndarray) -> None:
    ...
    def write_screen(
        ...
        color: colors.Color,
        screen_pos: List[int] | Tuple[int, int],
        text: str,
        align: str = "left",
        valign: str = "top",
    ):
...
class PastAction(
    collections.namedtuple(
        "PastAction", ["ability", "color", "pos", "time", "deadline"]
    )
):
...
class RendererHuman(object):
    ...
    def init_window(self):
        ...
        def add_surface(
            surf_type: SurfType,
            surf_loc: point.Rect,
            world_to_surf: transform.Transform,
            world_to_obs: transform.Transform,
        ...
        def check_eq(a: point.Point, b: point.Point) -> None:
        ...
        check_eq(
            self._world_to_world_tl.fwd_pt(point.Point(0, 0)),
            point.Point(0, self._map_size.y),
        )
        check_eq(
            self._world_to_world_tl.fwd_pt(point.Point(5, 10)),
        ...
        check_eq(
            self._world_tl_to_world_camera_rel.fwd_pt(self._map_size / 4),
            point.Point(0, 0),
        )
        check_eq(
        ...
        if self._feature_screen_px:
            ...
            check_eq(
                world_camera_rel_to_feature_screen.fwd_pt(point.Point(0, 0)),
            ...
            self._world_to_feature_minimap = transform.Chain(
                self._world_to_world_tl, world_tl_to_feature_minimap
            )
            self._world_to_feature_minimap_px = transform.Chain(
                self._world_to_feature_minimap, transform.PixelToCoord()
            )

            # These are confusing since self._playable is in world coords which is
            # (bl <= tr), but stored in a Rect that is (tl <= br).
            check_eq(
                self._world_to_feature_minimap.fwd_pt(self._playable.bl),
                point.Point(0, 0),
            )
            check_eq(
                self._world_to_feature_minimap.fwd_pt(self._playable.tr),
        ...
        if self._rgb_screen_px:
            ...
            check_eq(
                world_camera_rel_to_rgb_screen.fwd_pt(point.Point(0, 0)),
                self._rgb_screen_px / 2,
            )
            check_eq(
                world_camera_rel_to_rgb_screen.fwd_pt(point.Point(-0.5, -0.5) * 24),
                point.Point(0, 0),
            )
            ...
            self._world_to_rgb_screen_px = transform.Chain(
                self._world_to_rgb_screen, transform.PixelToCoord()
            )
            ...
            check_eq(
                world_tl_to_rgb_minimap.fwd_pt(point.Point(0, 0)), point.Point(0, 0)
            )
            check_eq(
                world_tl_to_rgb_minimap.fwd_pt(self._map_size),
                self._map_size.scale_max_size(self._rgb_minimap_px),
            )

            self._world_to_rgb_minimap = transform.Chain(
                self._world_to_world_tl, world_tl_to_rgb_minimap
            )
            self._world_to_rgb_minimap_px = transform.Chain(
                self._world_to_rgb_minimap, transform.PixelToCoord()
            )
        ...
        if self._render_rgb:
            ...
            add_surface(
                ...
                transform.Chain(  # surf
                    self._world_to_rgb_screen, rgb_screen_to_main_screen
                ),
            ...
            add_surface(
                ...
                point.Rect(minimap_offset, minimap_offset + minimap_size_px),
                transform.Chain(  # surf
                    self._world_to_rgb_minimap, rgb_minimap_to_main_minimap
                ),
            ...
            add_surface(
                ...
                point.Rect(point.origin, screen_size_px),
                transform.Chain(  # surf
                    self._world_to_feature_screen, feature_screen_to_main_screen
                ),
            ...
            add_surface(
                ...
                point.Rect(minimap_offset, minimap_offset + minimap_size_px),
                transform.Chain(  # surf
                    self._world_to_feature_minimap, feature_minimap_to_main_minimap
                ),
        ...
        help_size = (
            point.Point(
                (
                    ...
                    + max(len(s) for _, s in self.shortcuts)
                )
                * 0.4
                + 4,
                len(self.shortcuts) + 3,
            )
            * self._scale
        )
        help_rect = point.Rect(
            window_size_px / 2 - help_size / 2, window_size_px / 2 + help_size / 2
        )
    ...
    def get_mouse_pos(self, window_pos=None):
        ...
        for surf in reversed(self._surfaces):
            if surf.surf_type != SurfType.CHROME and surf.surf_rect.contains_point(
                window_pt
            ):
    ...
    def save_replay(
        self, run_config: RunConfig, controller: remote_controller.RemoteController
    ):
        if controller.status in (
            remote_controller.Status.in_game,
            ...
            if event.type == pygame.QUIT:
                ...
                if self._help:
                    ...
                    if self._rgb_screen_px and self._feature_screen_px:
                        ...
                        print(
                            "Rendering", self._render_rgb and "RGB" or "Feature Layers"
                        )
                        self.init_window()
                    ...
                    controller.act(
                        self.control_group(
                            self.cmd_group_keys[event.key], ctrl, shift, alt
                        )
                    )
                elif event.key in self.camera_actions:
                    if self._obs:
                        pt = point.Point.build(
                    ...
                    controller.observer_act(
                        self.camera_action_observer_player(
                    ...
                    if self._queued_action:
                        ...
                        cmds = self._abilities(
                            lambda cmd: cmd.hotkey == "escape"
                        )  # Cancel
                    ...
                    if not self._queued_action:
                        ...
                        cmds = self._abilities(
                            lambda cmd, n=new_cmd: (  # pylint: disable=g-long-lambda
                            ...
                            if len(cmds) == 1:
                                ...
                                if cmd.hotkey == self._queued_hotkey:
                                    if cmd.requires_point:
                                        ...
                                        controller.act(
                                            self.unit_action(cmd, None, shift)
                                        )
                ...
                if event.button == MouseButtons.LEFT and mouse_pos:
                    if self._queued_action:
                        controller.act(
                            self.unit_action(self._queued_action, mouse_pos, shift)
                        )
                    elif mouse_pos.surf.surf_type & SurfType.MINIMAP:
                        controller.act(self.camera_action(mouse_pos))
                        controller.observer_act(
                            self.camera_action_observer_pt(mouse_pos.world_pos)
                        )
                    else:
                ...
                if event.button == MouseButtons.LEFT and self._select_start:
                    if (
                        mouse_pos
                        and mouse_pos.surf.surf_type & SurfType.SCREEN
                        and mouse_pos.surf.surf_type
                        == self._select_start.surf.surf_type
                    ):
                        controller.act(
                            self.select_action(
    ...
    def camera_action(self, mouse_pos):
        ...
        mouse_pos.obs_pos.round().assign_to(action_spatial.camera_move.center_minimap)
        return action
        ...
        if self._raw_actions:
            ...
            if pos1.world_pos == pos2.world_pos:  # select a point
                for u, p in reversed(list(self._visible_units())):
                    if (
                        pos1.world_pos.contained_circle(p, u.radius)
                        and u.owner == player_id
                    ):
    ...
    def unit_action(self, cmd, pos, shift):
        ...
        if self._raw_actions:
            ...
            unit_command.unit_tags.extend(
                [
                    u.tag
                    for u, _ in self._visible_units()
    ...
    def _visible_units(self):
        ...
        for u in sorted(
            self._obs.observation.raw_data.units,
        ...
        for u, p in self._visible_units():
            if self._camera.intersects_circle(p, u.radius):
                ...
                if u.display_type == sc_raw.Placeholder:
                    surf.draw_circle(
                        colors.PLAYER_ABSOLUTE_PALETTE[u.owner] // 3, p, u.radius
                    )
                else:
                    surf.draw_circle(
                ...
                if u.is_selected:
                    ...
                    for o in u.orders:
                        ...
                        if target_point:
                            surf.draw_line(
                                colors.cyan * 0.75, start_point, target_point
                            )
                    ...
                    for rally in u.rally_targets:
                        surf.draw_line(
                            colors.cyan * 0.75, p, point.Point.build(rally.point)
                        )
    ...
    def draw_selection(self, surf):
        ...
        if select_start:
            ...
            if (
                mouse_pos
                and mouse_pos.surf.surf_type & SurfType.SCREEN
                and mouse_pos.surf.surf_type == select_start.surf.surf_type
            ):
    ...
    def draw_build_target(self, surf):
        ...
        def round_half(v, cond):
            return round(v - 0.5) + 0.5 if cond else round(v)
        ...
        if queued_action:
            ...
            if radius:
                ...
                if pos:
                    pos = point.Point(
                        round_half(pos.world_pos.x, (radius * 2) % 2),
    ...
    def draw_commands(self, surf):
        ...
        if action_count > 0:
            ...
            for cmd in sorted(
                self._abilities(lambda c: c.name != "Smart"), key=lambda c: c.name
            ):
    ...
    def draw_panel(self, surf):
        ...
        if ui.groups:
            ...
            for group in ui.groups:
                ...
                write(
                    (left + 3, y),
                    "%s %s" % (group.count, unit_name(group.leader_unit_type)),
                )
            next(line)
        ...
        if ui.HasField("single"):
            ...
            if (
                ui.single.attack_upgrade_level
                or ui.single.armor_upgrade_level
                or ui.single.shield_upgrade_level
            ):
    ...
    def prepare_actions(self, obs):
        ...
        for act in obs.actions:
            if (
                act.HasField("action_raw")
                and act.action_raw.HasField("unit_command")
                and act.action_raw.unit_command.HasField("target_world_space_pos")
            ):
            ...
            if act.HasField("action_feature_layer"):
                ...
                ) and act_fl.unit_selection_point.HasField("selection_screen_coord"):
                    pos = self._world_to_feature_screen_px.back_pt(
                        point.Point.build(
            ...
            if act.HasField("action_render"):
                ...
                if act_rgb.HasField(
                    "unit_selection_point"
                ) and act_rgb.unit_selection_point.HasField("selection_screen_coord"):
                    pos = self._world_to_rgb_screen_px.back_pt(
                        point.Point.build(
    ...
    def draw_mini_map(self, surf):
        ...
        if (
            ...
            and self._obs.observation.render_data.HasField("minimap")
        ):
            # Draw the rendered version.
            surf.blit_np_array(
                features.Feature.unpack_rgb_image(
            ...
            out = hmap_color * 0.6
            out[creep_mask, :] = (
                0.4 * out[creep_mask, :] + 0.6 * creep_color[creep_mask, :]
            )
            ...
            for radar in self._obs.observation.raw_data.radar:
                surf.draw_circle(
                    colors.white / 2, point.Point.build(radar.pos), radar.radius, 1
                )
    ...
    def draw_rendered_map(self, surf):
        ...
        surf.blit_np_array(
            features.Feature.unpack_rgb_image(self._obs.observation.render_data.map)
        )
    ...
    def render_obs(self, obs):
        ...
        self._update_camera(
            point.Point.build(self._obs.observation.raw_data.player.camera)
        )
