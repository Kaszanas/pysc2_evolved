...
def spatial(action, action_space: ActionSpace):
    ...
    if action_space == ActionSpace.FEATURES:
        ...
        raise ValueError(f"Unexpected value for action_space: {action_space}")


def no_op(action, action_space: ActionSpace):
    del action, action_space


def move_camera(action, action_space: ActionSpace, minimap):
    """Move the camera."""
    minimap.assign_to(spatial(action, action_space).camera_move.center_minimap)


def select_point(action, action_space: ActionSpace, select_point_act, screen):
...
def select_rect(action, action_space: ActionSpace, select_add, screen, screen2):
...
def select_idle_worker(action, action_space: ActionSpace, select_worker):
    """Select an idle worker."""
    del action_space
    action.action_ui.select_idle_worker.type = select_worker


def select_army(action, action_space: ActionSpace, select_add):
    """Select the entire army."""
    del action_space
    action.action_ui.select_army.selection_add = select_add


def select_warp_gates(action, action_space: ActionSpace, select_add):
    """Select all warp gates."""
    del action_space
    action.action_ui.select_warp_gates.selection_add = select_add


def select_larva(action, action_space: ActionSpace):
    """Select all larva."""
    del action_space
    action.action_ui.select_larva.SetInParent()  # Adds the empty proto field.


def select_unit(action, action_space: ActionSpace, select_unit_act, select_unit_id):
...
def cmd_screen(action, action_space: ActionSpace, ability_id, queued, screen):
...
def cmd_minimap(action, action_space: ActionSpace, ability_id, queued, minimap):
...
def autocast(action, action_space: ActionSpace, ability_id):
    """Toggle autocast."""
    del action_space
...
class FunctionCall(collections.namedtuple("FunctionCall", ["function", "arguments"])):
    ...
    # REVIEW: Type hints for this are horrible:
    # TODO: Use a more specific type for the function argument.
    @classmethod
    ...
    @classmethod
    def all_arguments(
        cls, function: Callable, arguments: Dict | Iterable, raw: bool = False
    ):
