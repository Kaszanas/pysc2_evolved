...
class Stat(object):
    ...
    @property
    def avg(self) -> Literal[0] | float:
        return 0 if self.num == 0 else self.sum / self.num

    @property
    def dev(self) -> Literal[0] | float:
        """Standard deviation."""
        if self.num == 0:
            return 0
...
class TracingStopWatchContext(StopWatchContext):
    ...
    def __exit__(self, *args, **kwargs):
        self._log(
            "<<< %s: %.6f secs" % (self._sw.cur_stack(), time.time() - self._start)
        )
...
class StopWatch(object):
    ...
    def custom(self, factory: Callable[[str], StopWatchContext]) -> None:
        self._factory = factory

    def __call__(self, name: str) -> StopWatchContext:
        return self._factory(name)

    def decorate(self, name_or_func: str | Callable) -> Callable:
        ...
        def decorator(name: str, func: Callable) -> Callable:
    ...
    def cur_stack(self) -> str:
        return ".".join(self._local.stack)

    def clear(self) -> None:
        self._times.clear()

    def add(self, name: str, duration: int | float) -> None:
    ...
    def str(self, threshold=0.1) -> str:
