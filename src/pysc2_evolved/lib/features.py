...
class Features(object):
    ...
    def transform_obs(
        ...
        if aif.rgb_dimensions:
            with sw("rgb_screen"):
                out["rgb_screen"] = Feature.unpack_rgb_image(
                    obs.observation.render_data.map
                ).astype(np.int32)
            with sw("rgb_minimap"):
                out["rgb_minimap"] = Feature.unpack_rgb_image(
                    obs.observation.render_data.minimap
                ).astype(np.int32)
        ...
        out["action_result"] = np.array(
            [o.result for o in obs.action_errors], dtype=np.int32
        )
    ...
    def transform_action(
        ...
        # Available?
        if not (skip_available or self._raw or func_id in self.available_actions(obs)):
            raise ValueError(
    ...
    def reverse_action(self, action: sc_pb.Action) -> actions.FunctionCall:
        ...
        def func_call_ability(ability_id, cmd_type, *args):
            ...
            if ability_id not in actions.ABILITY_IDS:
                logging.warning(
                    "Unknown ability_id: %s. This is probably dance or "
            ...
            raise ValueError(
                "Unknown ability_id: %s, type: %s. Likely a bug."
        ...
        if action.HasField("action_ui"):
            ...
            if act_ui.HasField("multi_panel"):
                return FUNCTIONS.select_unit(
                    act_ui.multi_panel.type - 1, act_ui.multi_panel.unit_index
                )
        ...
        if action.HasField("action_feature_layer") or action.HasField("action_render"):
            ...
            if act_sp.HasField("unit_command"):
                ...
                if cmd.HasField("target_screen_coord"):
                    ...
                    return func_call_ability(
                        cmd.ability_id, actions.cmd_screen, queue, coord
                    )
                elif cmd.HasField("target_minimap_coord"):
                    coord = point.Point.build(cmd.target_minimap_coord)
                    return func_call_ability(
                        cmd.ability_id, actions.cmd_minimap, queue, coord
                    )
                else:
        ...
        if action.HasField("action_raw"):
            ...
            if raw_act.HasField("unit_command"):
                ...
                if uc.HasField("target_world_space_pos"):
                    ...
                    return func_call_ability(
                        ability_id, actions.raw_cmd_pt, queue_command, unit_tags, coord
                    )
                else:
                    return func_call_ability(
                        ability_id, actions.raw_cmd, queue_command, unit_tags
                    )
