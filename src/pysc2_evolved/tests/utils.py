...
def get_units(obs, filter_fn=None, owner=None, unit_type=None, tag=None):
    ...
    for u in obs.observation.raw_data.units:
        if (
            (filter_fn is None or filter_fn(u))
            and (owner is None or u.owner == owner)
            and (unit_type is None or u.unit_type in unit_type)
            and (tag is None or u.tag == tag)
        ):
            out[u.tag] = u
    return out
...
class GameReplayTestCase(TestCase):
    ...
    def start_game(self, show_cloaked=True, disable_fog=False, players=2):
        ...
        join = sc_pb.RequestJoinGame(race=sc_common.Protoss, options=self._interface)
        if players == 2:
    ...
    def observe(self, disable_fog=False):
        return self._parallel.run(
            (c.observe, disable_fog)  # pytype: disable=attribute-error
            for c in self._controllers
    ...
    def move_camera(self, x, y):
        ...
        return self._parallel.run(
            (c.act, action) for c in self._controllers
        )  # pytype: disable=attribute-error

    @only_in_game
    def raw_unit_command(self, player, ability_id, unit_tags, pos=None, target=None):
        ...
        response = self._controllers[player].act(
            action
        )  # pytype: disable=attribute-error
    ...
    def debug(self, player=0, **kwargs):
        self._controllers[player].debug(
            [sc_debug.DebugCommand(**kwargs)]
        )  # pytype: disable=attribute-error
    ...
    def set_energy(self, tag, energy):
        self.debug(
            unit_value=sc_debug.DebugSetUnitValue(
                unit_value=sc_debug.DebugSetUnitValue.Energy, value=energy, unit_tag=tag
            )
        )
