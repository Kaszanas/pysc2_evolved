...
class ObsTest(utils.GameReplayTestCase):
    ...
    def test_hide_cloaked(self):
        ...
        self.assert_layers(
            screen2, dt, unit_type=0, unit_hit_points=0, unit_shields=0, cloaked=0
        )
    ...
    def test_show_cloaked(self):
        ...
        self.assert_unit(
            p2, display_type=sc_raw.Hidden, health=0, shield=0, cloak=sc_raw.Cloaked
        )
    ...
    def test_fog(self):
        ...
        def assert_visible(unit, display_type, alliance, cloak):
            self.assert_unit(
                unit, display_type=display_type, alliance=alliance, cloak=cloak
            )
        ...
        assert_visible(
            utils.get_unit(obs[0], unit_type=units.Protoss.Sentry),
            sc_raw.Visible,
            sc_raw.Self,
            sc_raw.NotCloaked,
        )
        assert_visible(
            utils.get_unit(obs[0], unit_type=units.Protoss.DarkTemplar),
            sc_raw.Visible,
            sc_raw.Self,
    ...
    def test_active(self):
        ...
        # Observer is idle.
        self.assert_unit(
            utils.get_unit(obs[0], unit_type=units.Protoss.Observer),
            display_type=sc_raw.Visible,
            is_active=False,
        )
        self.assert_unit(
            utils.get_unit(obs[1], unit_type=units.Protoss.Observer),
            display_type=sc_raw.Hidden,
            is_active=False,
        )
