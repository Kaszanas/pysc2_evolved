...
class _TestMixin:
    def assert_spec(self, array, shape, dtype):
        assert list(array.shape) == list(shape)
    ...
    def assert_mid_step(self, env):
        expected = env.next_timestep[0]._replace(step_type=environment.StepType.MID)
        timestep = env.step([mock.sentinel.action])
        self.assert_equal(timestep, [expected])

    def assert_last_step(self, env):
        expected = env.next_timestep[0]._replace(
            step_type=environment.StepType.LAST, discount=0.0
        )
        timestep = env.step([mock.sentinel.action])
        self.assert_equal(timestep, [expected])
    ...
    def _test_episode(self, env):
        env.next_timestep = [
            env.next_timestep[0]._replace(step_type=environment.StepType.MID)
        ]
        self.assert_first_step(env)

        for step in range(1, 10):
            env.next_timestep = [
                env.next_timestep[0]._replace(reward=step, discount=step / 10)
            ]
            self.assert_mid_step(env)

        env.next_timestep = [
            env.next_timestep[0]._replace(
...
@pytest.mark.minor
class TestTestEnvironment(_TestMixin):
    ...
    def test_observation_spec(self):
        assert self._env.observation_spec() == ({"mock": [10, 1]},)

    def test_action_spec(self):
        assert self._env.action_spec() == (mock.sentinel.action_spec,)

    def test_default_observation(self):
        observation = self._env._default_observation(self._env.observation_spec()[0], 0)
...
@pytest.mark.minor
class TestSC2TestEnv(_TestMixin):
    def test_episode(self):
    ...
    def test_screen_minimap_size(self):
        ...
        self.assert_spec(
            timestep[0].observation["feature_screen"],
            [len(features.SCREEN_FEATURES), 87, 84],
            np.int32,
        )
        self.assert_spec(
            timestep[0].observation["feature_minimap"],
    ...
    def test_feature_units_are_supported(self):
        ...
        assert "feature_units" in env.observation_spec()[0]
    ...
    def test_game_info(self):
        env = mock_sc2_env.SC2TestEnv(
            ...
            players=[
                sc2_env.Agent(sc2_env.Race.protoss, "player"),
                sc2_env.Bot(
                    sc2_env.Race.random,
        ...
        assert len(env.game_info) == 1
        assert env.game_info[0] == sc2api_pb2.ResponseGameInfo(
            start_raw=raw_pb2.StartRaw(
                map_size=common_pb2.Size2DI(
                    x=mock_sc2_env.DUMMY_MAP_SIZE, y=mock_sc2_env.DUMMY_MAP_SIZE
                )
            ),
