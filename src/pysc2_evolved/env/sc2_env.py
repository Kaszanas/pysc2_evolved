...
class SC2Env(environment.Base):
    ...
    def __init__(
        ...
        *,  # Force keyword arguments
        ...
        if isinstance(
            agent_interface_format, (AgentInterfaceFormat, sc_pb.InterfaceOptions)
        ):
    ...
    def _get_interface(interface_format, require_raw):
        ...
        interface = sc_pb.InterfaceOptions(
            raw=(
                aif.use_feature_units
                or aif.use_unit_counts
    ...
    def _launch_game(self):
        ...
        if self._battle_net_map:
            ...
            unavailable = [
                m.name for m in self._maps if m.battle_net not in available_maps
            ]
            if unavailable:
                raise ValueError(
    ...
    def _restart(self):
        if (
            len(self._players) == 1
            and len(self._players[0].race) == 1
            and len(self._maps) == 1
        ):
    ...
    def _step(self, step_mul=None):
        ...
        if not self._realtime:
            ...
            current_game_loop = self._send_delayed_actions(
                up_to_game_loop=target_game_loop, current_game_loop=self._episode_steps
            )

            self._step_to(
                game_loop=target_game_loop, current_game_loop=current_game_loop
            )
    ...
    def _get_observations(self, target_game_loop):
        ...
        if game_loop < target_game_loop and not any(o.player_result for o in self._obs):
            raise ValueError(
                (
                    "The game didn't advance to the expected game loop. "
                    "Expected: %s, got: %s"
                )
                % (target_game_loop, game_loop)
            )
...
def _get_score(agent_obs, score_index=0):
    if isinstance(agent_obs, sc_pb.ResponseObservation):
        if score_index != 0:
            raise ValueError(
                "Non-zero score index isn't supported for passthrough agents, currently"
            )
