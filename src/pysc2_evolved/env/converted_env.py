    ...
    def step(self, actions) -> dm_env.TimeStep:
        ...
        for i, (action, converter) in enumerate(zip(actions, self._converters)):
            if action is None:
                ...
                if (
                    self._target_game_loops[i] > self._game_loop
                    and not self._allow_out_of_turn_actions
                ):
        ...
        timestep = self._convert_timesteps(self._env.step(converted_actions, min_delay))
...
def make_streams(converted_environment: ConvertedEnvironment):
    ...
    return [_Stream(p, environment) for p in range(converted_environment.num_players())]
...
def get_environment_spec(
    ...
    obs_spec = tree.map_structure(squeeze_spec_if_necessary, cvr.observation_spec())
...
def make_converter_factories(
    ...
    return [functools.partial(converter_factory, s) for s in all_converter_settings]
