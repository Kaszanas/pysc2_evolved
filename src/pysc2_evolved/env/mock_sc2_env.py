...
class _TestEnvironment(environment.Base):
    ...
    def step(self, actions, step_mul=None):
        ...
        if len(actions) != self._num_agents:
            raise ValueError(
                "Expected %d actions, received %d." % (self._num_agents, len(actions))
            )
        ...
        for timestep in self.next_timestep:
            if step_type is environment.StepType.FIRST:
                timesteps.append(
                    timestep._replace(step_type=step_type, reward=0.0, discount=0.0)
                )
            elif step_type is environment.StepType.LAST:
                timesteps.append(timestep._replace(step_type=step_type, discount=0.0))
            else:
...
class SC2TestEnv(_TestEnvironment):
    ...
    def __init__(
        ...
        *,
        ...
        self._features = [
            features.features_from_game_info(game_info=g, agent_interface_format=aif)
