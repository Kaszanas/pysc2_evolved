...
#include "pysc2_evolved/env/converter/cc/visual_converter.h"
...
#include "pysc2_evolved/env/converter/cc/visual_actions.h"
...
dm_env_rpc::v1::Tensor AvailableActions(const SC2APIProtocol::Observation& obs,
  ...
  for (const auto& available_ability : obs.abilities()) {
    ...
    CHECK(found_applicable)
        << "Failed to find applicable action for "
        << available_ability.ShortDebugString();
  }
...
VisualConverter::DecodeAction(
  ...
  pysc2_evolved::ActionContext action_context = {
      settings_.visual_settings().screen().x(), settings_.minimap().x(),
      settings_.num_action_types()};

  return pysc2_evolved::Decode(action, action_context);
}
