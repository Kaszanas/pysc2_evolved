...
class ConverterWrapper {
 ...
 private:
  pysc2_evolved::Converter converter_;

 public:
  ConverterWrapper(pysc2_evolved::Converter converter)
  ...
  std::map<std::string, pybind11::bytes> ConvertObservation(
    ...
    pysc2_evolved::Observation deserialized_obs;
  ...
  pybind11::bytes ConvertAction(
    ...
    absl::StatusOr<pysc2_evolved::Action> converted_action_or;
...
ConverterWrapper MakeConverterWrapper(const std::string& settings,
  ...
  // Deserialize strings.
  pysc2_evolved::EnvironmentInfo env_info;
  pysc2_evolved::ConverterSettings converter_settings;
  converter_settings.ParseFromString(settings);
  env_info.ParseFromString(environment_info);
  absl::StatusOr<pysc2_evolved::Converter> converter_or =
      pysc2_evolved::MakeConverter(converter_settings, env_info);
  if (!converter_or.ok()) {
