...
$ python -m pysc2_evolved.bin.play_vs_agent --human --map <map> --remote <agent ip>

And on the machine the agent plays on:
$ python -m pysc2_evolved.bin.play_vs_agent --agent <import path>
...
flags.DEFINE_string("remote", None, "Where to set up the ssh tunnels to the client.")
...
def human():
    ...
    try:
        ...
        print("-" * 80)
        print("Join: play_vs_agent --host %s --config_port %s" % (proc.host, tcp_port))
        print("-" * 80)
