...
FLAGS = flags.FLAGS
flags.DEFINE_string("bnet_base", None, "Path to a Battle.net directory to update.")
...
def update_battle_net_cache(replays, bnet_base):
    ...
    for replay_path in replays:
        ...
        details_bytes = extracted.get(b"replay.details") or extracted.get(
            b"replay.details.backup"
        )
...
def main(argv):
    ...
    bnet_base = FLAGS.bnet_base or os.path.join(
        run_configs.get().data_dir, "Battle.net"
    )
