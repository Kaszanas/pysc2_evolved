...
flags.DEFINE_integer(
    "truncate",
    0,
    "Number of characters to truncate diff values to, or 0 for no truncation.",
)
...
def main(argv):
    ...
    start_replay = sc_pb.RequestStartReplay(
        replay_data=replay_data, options=interface, observed_player_id=1, realtime=False
    )
    ...
    for target in targets:
        ...
        with timer("launch"):
            if _is_remote(target):
                ...
                proc = run_configs.get(version=version._replace(binary=target)).start(
                    want_rgb=False
                )
                procs.append(proc)
    ...
    try:
        ...
        for _ in range(FLAGS.count):
            ...
            if FLAGS.diff:
                ...
                if any(diffs.values()):
                    print(
                        (" Diff on step: %s " % obs[0].observation.game_loop).center(
                            80, "-"
                        )
                    )
                    for i, diff in diffs.items():
