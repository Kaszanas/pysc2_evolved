...
To play on ladder:
  $ python -m pysc2_evolved.bin.agent_remote --agent <import path> \
      --host_port <GamePort> --lan_port <StartPort>

To play vs humans:
  $ python -m pysc2_evolved.bin.agent_remote --human --map <MapName>
...
from pysc2_evolved.env import remote_sc2_env, run_loop, sc2_env
from pysc2_evolved.lib import point_flag, portspicker, renderer_human

FLAGS = flags.FLAGS
flags.DEFINE_bool(
    "render", platform.system() == "Linux", "Whether to render with pygame."
)
...
flags.DEFINE_string(
    "user_name", getpass.getuser(), "Name of the human player for replays."
)
flags.DEFINE_enum(
    "user_race",
    "random",
...
def human():
    ...
    if not FLAGS.rgb_screen_size or not FLAGS.rgb_minimap_size:
        logging.info(
            "Use --rgb_screen_size and --rgb_minimap_size if you want rgb observations."
        )

    ports = portspicker.pick_contiguous_unused_ports(4)  # 2 * num_players
    host_proc = run_config.start(
        extra_ports=ports,
        host=FLAGS.host,
        timeout_seconds=FLAGS.timeout_seconds,
        window_loc=(50, 50),
    )
    client_proc = run_config.start(
        extra_ports=ports, host=FLAGS.host, connect=False, window_loc=(700, 50)
    )
    ...
    print("-" * 80)
    print(
        "Join host: agent_remote --map %s --host %s --host_port %s "
