...
def get_data():
    ...
    with run_config.start(want_rgb=False) as controller:
        ...
        join = sc_pb.RequestJoinGame(
            race=sc_common.Random, options=sc_pb.InterfaceOptions(raw=True)
        )
...
def generate_name(ability):
    return ability.friendly_name or ability.button_name or ability.link_name
...
def generate_csv(data):
    ...
    for ability in sorted(data.abilities.values(), key=lambda a: sort_key(data, a)):
        ab_id = ability.ability_id
        if ab_id in skip_abilities or (
            ab_id not in data.general_abilities and ab_id not in used_abilities
        ):
            continue
...
def generate_py_abilities(data):
    ...
    for ability in sorted(data.abilities.values(), key=lambda a: sort_key(data, a)):
        ...
        if ability.target in (
            sc_data.AbilityData.Target.Value("None"),
