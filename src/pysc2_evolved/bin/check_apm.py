...
def get_units(obs, filter_fn=None, owner=None, unit_type=None, tag=None):
    ...
    return {
        u.tag: u
        for u in obs.observation.raw_data.units  # pylint: disable=g-complex-comprehension
        if (
...
class Env(object):
    ...
    def fl_action(self, obs, act, *args):
        return self._controller.act(
            self._features.transform_action(
    ...
    def check_apm(self, name):
        ...
        create = sc_pb.RequestCreateGame(
            random_seed=1,
            local_map=sc_pb.LocalMap(map_path=self._map_path, map_data=self._map_data),
        )
        create.player_setup.add(type=sc_pb.Participant)
        create.player_setup.add(
            type=sc_pb.Computer, race=sc_common.Protoss, difficulty=sc_pb.VeryEasy
        )
...
def main(unused_argv):
    ...
    def random_probe_loc(obs):
        return random.choice(
            _xy_locs(env.fl_obs(obs).feature_screen.unit_type == units.Protoss.Probe)
        )
    ...
    for i, obs in env.check_apm("raw move, random probe, random location"):
        env.raw_unit_command("Move_screen", random_probe_tag(obs), rand_world_coord())
